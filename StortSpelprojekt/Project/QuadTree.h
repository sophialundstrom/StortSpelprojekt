...
struct OrthographicCollider
{
    ...
    void Update(DirectionalLight dirLight, Vector3 playerPosition)
    {   
        float downSacle = 0.75f;
        float lightRange = 100;

        Vector3 direction = dirLight.data.direction;
        Vector3 lightPos = playerPosition + -direction * lightRange;

        float pitch = asinf(direction.y);
        float yaw = atan2f(direction.x, direction.z);
        Quaternion camDirQ = Quaternion::CreateFromYawPitchRoll(yaw, pitch, 0);
        bounds.Extents = { lightRange * downSacle, lightRange * downSacle, lightRange };
