...
float4 main(PS_INPUT input) : SV_TARGET
{
    ...
    //RESULT
    return lerp((diffuseTexture.Sample(wrapSampler, input.texCoords)) * (saturate(LightCalculation(input.worldPosition, input.normal, diffuse, specular, ambient, directionalLight, cameraPosition))), fogColor, fogFactor) * shadow;
