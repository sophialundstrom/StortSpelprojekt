...
public:
	...
	void AddButton(D2D_VECTOR_2F position, const std::string name, UINT width, UINT height, UI::COLOR color, std::function<void()> onClickFunction = NULL, std::function<void()> onHoverFunction = NULL, bool visible = true) { buttons.emplace(name, Button(position, width, height, UI::Inst().GetBrush(color), onClickFunction, onHoverFunction, visible)); }
	...
	void AddText(D2D_VECTOR_2F position, const std::string name, const std::string string, float width, float height, UI::COLOR color, UI::TEXTFORMAT format, bool visible = true) { texts.emplace(name, Text(std::wstring(string.begin(), string.end()), position, width, height, UI::Inst().GetTextFormat(format), UI::Inst().GetBrush(color), visible)); }
